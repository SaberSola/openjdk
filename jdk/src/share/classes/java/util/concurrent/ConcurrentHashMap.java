...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
                    ...
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) {          //表示该节点是链表结构
                            ...
                            for (Node<K,V> e = f;; ++binCount) {  //遍历链表
                                ...
                                if (e.hash == hash &&
                                    ((ek = e.key) == key ||
                                     (ek != null && key.equals(ek)))) {         //这里表示 key key的hash 完全相同 所以是相同的key 覆盖value
                                    oldVal = e.val;
                                    if (!onlyIfAbsent)
                                        e.val = value;
                                    break;
    ...
    final Node<K,V>[] helpTransfer(Node<K,V>[] tab, Node<K,V> f) {
        Node<K,V>[] nextTab;  //空链表 表示扩容后的table
        int sc;  //
    ...
    private final void transfer(Node<K,V>[] tab, Node<K,V>[] nextTab) {
        int n = tab.length, stride;    //这里代表 声明了 n = tab.length  int 变量 stride

        //将 length / 8 然后除以 CPU核心数。如果得到的结果小于 16，那么就使用 16。
        if ((stride = (NCPU > 1) ? (n >>> 3) / NCPU : n) < MIN_TRANSFER_STRIDE) //  治理的目的是让每个cpu处理的筒子数目一样多，如果筒子数目比较少的话默认一个线程处理16个
            stride = MIN_TRANSFER_STRIDE; // subdivide range
        if (nextTab == null) {            // initiating    //这里表示新的 nextTab为null 也就是未初始化的时候
            try {
                @SuppressWarnings("unchecked")
                Node<K,V>[] nt = (Node<K,V>[])new Node<?,?>[n << 1];
                nextTab = nt;                                        //扩容2倍 赋值给nexTab
            } catch (Throwable ex) {      // try to cope with OOME
                sizeCtl = Integer.MAX_VALUE;                         //扩容失败的话 sizeCtl使用最大值
                return;
            }
            nextTable = nextTab;                  //将扩容2倍后的nextab 赋值给全局变量nextTable
            transferIndex = n;                    //更新扩容后的新下标 就是oldtab的length
        }
        int nextn = nextTab.length;               //新table lenth 赋值给nextn
        ForwardingNode<K,V> fwd = new ForwardingNode<K,V>(nextTab); //创建一个fwd节点 进行站位 当别的线程发现这个节点是fwd节点的时候会跳过
        //首次推进为 true，如果等于 true，说明需要再次推进一个下标（i--），反之，如果是 false，那么就不能推进下标，需要将当前的下标处理完毕才能继续推进
        boolean advance = true;
        //退出循环的标志  true 表示结束此方法
        boolean finishing = false; // to ensure sweep before committing nextTab
        //死循环，i 表示下标  bound 表示一个线程所能处理的边界
        for (int i = 0, bound = 0;;) {
            Node<K,V> f;  //相当于临时节点
            int fh;       //节点的hash值
            ...
            while (advance) {
                ...
                if (--i >= bound || finishing)
                    //这里改为false 是为了防止在没有成功处理一个桶的情况下却进行了推进
                    advance = false;
            ...
            if (i < 0 || i >= n || i + n >= nextn) {
                ...
                if (finishing) {   //如果完成了扩容
                    nextTable = null;  //成员变量仍然置为null 供下次扩容使用
                    table = nextTab;   //新的hashtable 赋值给 table node组
                    sizeCtl = (n << 1) - (n >>> 1);  //跟新阀值
                    return;           //return
                }
                //这里表示扩容没有完成
                if (U.compareAndSwapInt(this, SIZECTL, sc = sizeCtl, sc - 1)) {  // 尝试将 sc -1. 表示这个线程结束帮助扩容了，将 sc 的低 16 位减一。
                    ...
                    i = n; // recheck before commit   //再次循环检查整张表
                }
            }
                ...
                synchronized (f) {
                    ...
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) {
                            ...
                            int runBit = fh & n;          //runBit 只有俩种结果 1,0
                            Node<K,V> lastRun = f;        //尾节点，且和头节点的 hash 值取于不相等 为什么是尾节点 因为 是倒叙遍历的 i--
                            for (Node<K,V> p = f.next; p != null; p = p.next) { //遍历这个筒子
                                int b = p.hash & n;          //对筒子的每个节点进行length & 运算
                                if (b != runBit) {           // 如果节点的 hash 值和首节点的 hash 值取于结果不同
                                    runBit = b;              // 更新 runBit，用于下面判断 lastRun 该赋值给 ln 还是 hn。
                                    lastRun = p;             // 这个 lastRun 保证后面的节点与自己的取于值相同，避免后面没有必要的循环
                                }
                            }
                            if (runBit == 0) {             // 运算为0的话 置为低位桶
                                ln = lastRun;
                                hn = null;
                            }
                            else {                        //否则为与高位桶
